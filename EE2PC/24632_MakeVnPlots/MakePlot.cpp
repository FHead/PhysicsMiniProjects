...
void MakeSquareDelta(TGraphAsymmErrors &G1, TGraphAsymmErrors &G2)
{
   for(int i = 0; i < G1.GetN(); i++)
   {
      ...
      double NewY = Sign(Y) * Y * Y - Sign(YShift) * YShift * YShift;
      NewY = Sign(NewY) * sqrt(fabs(NewY));
      
      cout << Y << " " << YShift << " " << NewY << endl;

      double NewEYLow = EYLow * Y / fabs(NewY);
      double NewEYHigh = EYHigh * Y / fabs(NewY);

      G1.SetPoint(i, X, NewY);
      G1.SetPointError(i, EXLow, EXHigh, NewEYLow, NewEYHigh);
   }
}
